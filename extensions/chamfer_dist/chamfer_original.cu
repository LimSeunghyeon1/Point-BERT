...
__global__ void chamfer_dist_kernel_double(int batch_size,
                                           int n,
                                           const double* xyz1,
                                           int m,
                                           const double* xyz2,
...
std::vector<torch::Tensor> chamfer_cuda_forward_double(torch::Tensor xyz1,
  ...
  chamfer_dist_kernel_double<<<dim3(32, 16, 1), 512>>>(
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("Error in chamfer_cuda_forward_double: %s\n", cudaGetErrorString(err));
  }
  return {dist1, dist2, idx1, idx2};
}
...
__global__ void chamfer_dist_grad_kernel_double(int b,
                                                int n,
                                                const double* xyz1,
                                                int m,
                                                const double* xyz2,
...
std::vector<torch::Tensor> chamfer_cuda_backward_double(torch::Tensor xyz1,
  ...
  chamfer_dist_grad_kernel_double<<<dim3(1, 16, 1), 256>>>(
    batch_size, n, xyz1.data_ptr<double>(), m, xyz2.data_ptr<double>(),
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("Error in chamfer_cuda_backward_double: %s\n", cudaGetErrorString(err));
  }
  return {grad_xyz1, grad_xyz2};
}